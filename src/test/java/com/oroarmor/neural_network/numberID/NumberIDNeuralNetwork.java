...
import com.oroarmor.neural_network.lib.layer.FeedFowardLayer;
...
import com.oroarmor.neural_network.lib.network.NetworkSaver;
import com.oroarmor.neural_network.lib.network.NeuralNetwork;
import com.oroarmor.neural_network.lib.training.GetData;
import com.oroarmor.neural_network.lib.training.Tester;
import com.oroarmor.neural_network.lib.training.Trainer;
import com.oroarmor.neural_network.lib.training.models.TotalError;
...
public class NumberIDNeuralNetwork extends PApplet {
	...
	public void setup() {
		...
		if (numberIDNetwork == null || reset) {
			...
			numberIDNetwork.addLayer(new FeedFowardLayer<>(64, MatrixType.CPU));
			numberIDNetwork.addLayer(new FeedFowardLayer<>(32, MatrixType.CPU));
			numberIDNetwork.addLayer(new FeedFowardLayer<>(16, MatrixType.CPU));
			numberIDNetwork.addLayer(new FeedFowardLayer<>(16, MatrixType.CPU));
			numberIDNetwork.addLayer(new FeedFowardLayer<>(10, MatrixType.CPU));
		}
		test();
		train();
		test();
...
//			}
	...
	public void train() {
		...
		for (int repeats = 0; repeats < 1; repeats++) {
			...
			for (int i = 0; i < threads; i++) {
				GetData<CPUMatrix> getInputs = new GetData<>(
				...
				GetData<CPUMatrix> getOutputs = new GetData<>(
						new String[] { i + "", numImages / threads + "" }) {
					@Override
					public CPUMatrix getData(String[] args) {
	...
	public void test() {
		...
		for (int i = 0; i < threads; i++) {
			...
			GetData<CPUMatrix> getInputs = new GetData<>(new String[] { i + "", numImages / threads + "" }) {
				@Override
				public CPUMatrix getData(String[] args) {
					return getImageData(loadImage("C:\\oroarmor\\numberID\\test\\images\\"
							+ (Integer.parseInt(globalArgs[0]) * Integer.parseInt(globalArgs[1])
			...
			GetData<CPUMatrix> getOutputs = new GetData<>(new String[] { i + "", numImages / threads + "" }) {
				@Override
				public CPUMatrix getData(String[] args) {
